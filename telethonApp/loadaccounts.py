...
async def check_session(session_file_path: str) -> dict | None:
    ...
    """
    session_name = os.path.basename(session_file_path).replace('.session', '') # Получаем имя сессии из пути
    ...
    try:
        ...
        if me:
             logger.info(f"[{session_name}] Успешно подключен аккаунт: {me.first_name} (@{me.username}), ID: {me.id}, Phone: {me.phone}")
             account_info = {
             ...
             logger.error(f"[{session_name}] Не удалось получить информацию об аккаунте (get_me вернул None).")

    except UserDeactivatedBanError:
        logger.error(f"[{session_name}] Аккаунт заблокирован (UserDeactivatedBanError).")
    except AuthKeyError:
        logger.error(f"[{session_name}] Файл сессии некорректный или ключ авторизации недействителен (AuthKeyError).")
    except ConnectionError as e:
         logger.error(f"[{session_name}] Ошибка подключения: {e}")
    except FloodWaitError as e:
         logger.error(f"[{session_name}] Слишком много запросов (FloodWaitError): подождите {e.seconds} секунд.")
    except Exception as e:
        logger.error(f"[{session_name}] Непредвиденная ошибка при проверке сессии: {e}", exc_info=True) # Логируем traceback
    finally:
...
async def process_sessions():
    ...
    try:
        all_session_files = [
            f for f in os.listdir(BASE_SESSION_DIR)
            if f.endswith('.session') and os.path.isfile(os.path.join(BASE_SESSION_DIR, f))
        ]
        ...
        try:
            ...
            for part in parts:
                ...
                if 0 <= index < len(all_session_files):
                    ...
                    print(f"Ошибка: Номер {part} вне допустимого диапазона (1-{len(all_session_files)}).")
                    invalid_input = True
                    break
        ...
        async def check_and_process(s_path, s_purpose, t_dir, s_file_name):
            ...
            if account_info:
                ...
                try:
                    ...
                    logger.info(f"[{account_info['session']}] Файл сессии перемещен в {t_dir}")
                    ...
                    logger.info(f"Аккаунт {account_info['phone']} ({account_info['session']}) успешно зарегистрирован как '{s_purpose}'.")
                    return True # Успех

                except OSError as e:
                    logger.error(f"[{account_info['session']}] Не удалось переместить файл сессии из {account_info['full_path']} в {target_path}: {e}")
                    print(f"Ошибка: Не удалось переместить файл {s_file_name}. Подробности в логах.")
                    return False # Неудача перемещения
                except Exception as e:
                     logger.error(f"[{account_info['session']}] Ошибка при сохранении в БД или другой операции: {e}", exc_info=True)
                     print(f"Ошибка: Произошла ошибка при обработке аккаунта {s_file_name}. Подробности в логах.")
                     # Решаем, нужно ли пытаться переместить файл обратно или оставить как есть
                     return False # Неудача обработки
            else:
                logger.warning(f"Сессия из файла {s_file_name} не прошла проверку (невалидна, заблокирована или не авторизована). Файл не перемещен.")
                print(f"\033[96m[?]\033[0m \033[92m Предупреждение: Сессия {s_file_name} невалидна или не авторизована. Пропущена.\033[0m ")
    ...
    results = await asyncio.gather(*tasks, return_exceptions=True) # Ловим исключения из корутин

    # 5. Подводим итоги
    for result in results:
         if isinstance(result, Exception):
             # Если gather поймал исключение из самой корутины check_and_process
             logger.error(f"Непредвиденная ошибка в задаче обработки сессии: {result}", exc_info=result)
             failed_count += 1
         elif result is True:
             processed_count += 1
    ...
    print(f"\033[96m[?]\033[0m \033[92mУспешно обработано и перемещено: {processed_count} \033[0m ")
    print(f"\033[96m[?]\033[0m \033[92m Пропущено (невалидные/неавторизованные): {len(selected_indices) - processed_count - failed_count}\033[0m ")
    print(f"\033[96m[?]\033[0m \033[92m Возникло ошибок при обработке (перемещение/БД): {failed_count}\033[0m ")
    print(f"\033[96m[?]\033[0m \033[92m Осталось необработанных сессий в '{BASE_SESSION_DIR}': {len(all_session_files) - len(selected_indices)}\033[0m ")
