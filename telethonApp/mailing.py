...
async def send_message():
    ...
    print(
        "\033[96m[?]\033[0m \033[92mВведите количество сообщений на аккаунт (по умолчанию 4)(Enter - skip): \033[0m "
    )
    message_limit = input("\033[96m └─>\033[0m \033[92m\033[0m ")
    MESSAGE_LIMIT = int(message_limit) if message_limit else MESSAGE_LIMIT

    print(
        "\033[96m[?]\033[0m \033[92mВведите количество лидов для смены сообщения (по умолчанию 150): \033[0m "
    )
    ...
    print(
        "\033[96m[?]\033[0m \033[92mВведите интервал между сообщениями (по умолчанию 5с):  \033[0m "
    )
    ...
    lead_index = next(
        (
            ...
            if lead["telegram_id"] == last_processed_lead_id
        ),
        0,
    )
        ...
        try:
            ...
            try:
                ...
                await update_account(
                    {"phone": account["phone"]},
                    send_count_message=account.get("send_count_message", 0) + 1,
                )
                await update_lead(
